...
import { api, API, fmtMXN, fmtDate } from "../lib/api";
...
export default function ContractViewer({ contract, open, onOpenChange, onSigned }) {
  ...
  const handleDownload = async () => {
    try {
      const res = await fetch(`${API}/contracts/${contract.contract_id}/pdf`, { credentials: "include" });
      if (!res.ok) throw new Error("PDF error");
      const blob = await res.blob();
      const url = URL.createObjectURL(blob);
      const a = document.createElement("a");
      a.href = url;
      a.download = `contrato_${contract.contract_id}.pdf`;
